...
// small arrow child mesh makes the current facing visible), replacing the old
// separate drag-a-target-point mechanism. Only one light is ever attached to the
// shared gizmo at a time, toggled per-card via Move/Rotate buttons (same pattern
// StepRayOptics uses for per-body transform gizmos).

let lights = [];
const lightObjects = new Map(); // id -> { group, marker, glow, arrow }

// Personal material library — name -> material, pushed down from the C# host (a flat
// JSON-file-per-material store shared across every document; see
// addin/InventorRayOptics/MaterialLibrary.cs). Populated via the "materialLibraryUpdated"
// host message; buildMaterialUI lists these in a "My Materials" dropdown group.
let materialLibrary = {};
...
function updateSceneScale() {
  ...
  // no longer exists). No 10-unit floor here (unlike sceneDiag below) — that floor was
  // silently overpowering proportionality for any real part smaller than ~37 units
  // (0.3/0.008), which is most real engineering parts. Only guard against literal zero
  // (an empty/degenerate model), not "small".
  ...
  // model's — a part with several small bodies spaced far apart has a huge combined
  // model diagonal (dominated by the spacing between bodies, not any one body's actual
  // size), which is exactly what made every light's marker/arrow look oversized
  // relative to the small body it actually sits on.
  for (const light of lights) {
    const obj = lightObjects.get(light.id);
    if (!obj) continue;
...
// both call sites so this isn't duplicated, and rendered in one place per light instead
// of two — this used to also render, unconditionally, near the bottom of buildLightUI,
// which is what produced a duplicate "Emission" control on body lights.
...
function buildBodyLightUI(light, container) {
  ...
  if (light.subMode === 'point') {
    ...
    // Force out any stale value from a previous subMode/an older saved-settings
    // snapshot rather than silently keeping it around unused.
    ...
    // Custom Profile (the curve editor, no slider) — mutually exclusive, so switching
    // away from Custom Profile clears it rather than leaving it toggled separately.
    const emissionRow = document.createElement('div');
    emissionRow.className = 'slider-row';
    const emissionLabel = document.createElement('span');
    emissionLabel.textContent = 'Emission';
...
function buildMaterialUI(body, container) {
  ...
  select.addEventListener('change', () => {
    ...
    if (kind === 'preset') {
      ...
      // material may have been a preset of this same type (e.g. a YAG phosphor
      // preset is 'cauchy'), and selecting "(custom)" should drop its phosphor
      // config, not silently inherit it. Seed with generic starting coefficients
      // that don't exactly equal any named preset — otherwise the dropdown
      // immediately relabels "(custom)" back to whichever preset the seed happens
      // to match (see materialSelectValue)
...
function clearModel() {
  ...
  clearRays();
  emitterSelectionLightId = null;
  // body-based lights reference bodyIds from the model being discarded — remove them
  for (const light of [...lights]) {
    if (light.kind === 'body') removeLight(light.id);
  }
...
async function loadModelFromUrl(url) {
  ...
  try {
    ...
    // best-effort: the add-in writes this alongside model.step for part documents (see
    // OpticsDockable.ShowFor/StepExporter.TryGetBodyNames), index-matched to the same
    // TopAbs_SOLID order buildFaceTable just walked. 404s silently for assemblies, and
    // for the standalone browser dev harness (no such file exists there) — both fall
    // back to the generic "Body N" label in rebuildBodyList.
    try {
      // the URL constructor's base argument must itself be absolute — resolve `url`
      // against the page's own location first so a relative model URL (the standalone
      // dev-server path) works the same as the absolute one WebView2 always passes
      const modelUrl = new URL(url, document.baseURI);
      const res = await fetch(new URL('bodyNames.json', modelUrl).href + '?t=' + Date.now());
      if (res.ok) {
        const names = await res.json();
        faceTable.bodies.forEach((b, i) => { if (names[i]) b.name = names[i]; });
      }
    } catch {
      // no names available — generic labels are fine
    }
...
function generateBodyEmissionRays(light, rayCount) {
  ...
  // 'profile' (the "Custom Profile" dropdown option for 'surfaces') always implies one,
  // 'maxAngle' only has one when the separate profile checkbox under it is checked.
  ...
  for (const light of lights) {
    ...
    if (light.kind === 'body' && light.subMode !== 'point') {
      ...
      if (light.angularProfile) {
        ...
        // instead of a uniform-density fan weighted after the fact. Always spans the
        // full fixed 0-90° domain the curve is plotted against — coneAngle stops
        // applying once a profile is active.
    ...
    for (const { wavelength: wl, weight } of wavelengths) {
      ...
      // phosphor-converted ray (Phase 6/7) carries a different wavelength than the
      // pass it started in, and its subsequent IOR lookups must reflect that
      ...
      // same "dim, don't drop" treatment as an angular profile, see lights.js's
      // traceWavelengths) so segments from different lights/ray counts/intensities
      // combine correctly in one buildRayLines call. Each segment already carries its
      // own rgb (from the ray's current wavelength at that point — see brepTracer.js),
      // not a per-batch color, so a phosphor-converted ray's tail renders in its new
      // color even though it started this pass at wavelength `wl`.
...
// Captures everything a "Save Settings" ribbon click needs to restore later: every
// body's material/light-source role, every face's reflectivity/dichroic/phosphor
// override, every light's full config plus its 3D transform (position/aim), and the
// global tracing parameters. Deliberately excludes the default light (isDefault) and
// each light's `id` — both are session-local bookkeeping that applySettings
// regenerates fresh rather than trusting a snapshot from a possibly-different session.
...
// Reverses serializeSettings. Matches bodies/faces by their stable occt.js-assigned
// id (consistent as long as the model itself hasn't changed shape since saving); any
// saved id with no live counterpart (a re-exported model with a different body/face
// count) is silently skipped rather than erroring.
...
// WebView2 panel is initialized (see addin/InventorRayOptics/OpticsDockable.cs). Also
// handles the material-library push and the save/load-settings request/reply pair
// driven by the "Ray Optics" ribbon tab's buttons (see StandardAddInServer.cs) — every
// reply is posted back as {requestId, data}, matched to the pending C#-side request by
// requestId; see OpticsDockable.RequestAsync.
function postReplyToHost(requestId, data) {
  window.chrome.webview.postMessage(JSON.stringify({ requestId, data }));
}
if (window.chrome && window.chrome.webview) {
  window.chrome.webview.addEventListener('message', (e) => {
    const msg = e.data;
    if (!msg || !msg.type) return;
    if (msg.type === 'loadStep' && msg.url) {
      if (msg.version) $('build-version').textContent = `build ${msg.version}`;
      loadModelFromUrl(msg.url);
    } else if (msg.type === 'materialLibraryUpdated') {
      materialLibrary = msg.library || {};
      if (faceTable) rebuildBodyList();
    } else if (msg.type === 'listBodiesRequest') {
      const bodies = faceTable ? faceTable.bodies.filter((b) => !b.isLightSource).map((b) => ({
        id: b.id, label: b.name || `Body ${b.id + 1}`, material: b.material,
      })) : [];
      postReplyToHost(msg.requestId, bodies);
    } else if (msg.type === 'requestSettingsSnapshot') {
      postReplyToHost(msg.requestId, serializeSettings());
...
window.__iro = {
  ...
  selectFace, trace, addLight, removeLight, loadModelFromUrl,
  serializeSettings, applySettings, setTracingEnabled,
  get materialLibrary() { return materialLibrary; },
  setMaterialLibrary(lib) { materialLibrary = lib; if (faceTable) rebuildBodyList(); },
};
